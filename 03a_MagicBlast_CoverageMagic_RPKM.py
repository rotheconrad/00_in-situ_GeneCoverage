...
This RPKM script contains added functionality to calculate and 
write separate files for gene and intergene relative abundance and
RPKM-ish values in addition to the TAD80. Relative abundance for genes
is calculated as described above. RPKM is calculated as:

Reads (base pairs) per kilo base per million mapped reads (base pairs)

Formula:

base pairs recruited to gene /
( (Gene Length/1000) * (base pairs in metagenome/1,000,000) )
...
def get_gene_tad(gn_tad, tad, tmb):
    """ reads through gn_tad and returns dict of tads by gene 
    tmb is total metagenome base pairs to calc rela of genes """
    ...
    for k, v in gn_tad.items():
        ...
        gene_rela[k] = (sum(v) / tmb) * 100
        gene_rpkm[k] = sum(v) / ((len(v)/1000) * (tmb/1000000))

    return gene_tad, gene_breadth, gene_rela, gene_rpkm
...
def calc_gene_stats(
    gn_tad, gn_anir, gn_len, tad, outpre, precision, tmb
    ):
    """Calculate ANIr, TAD and breadth and write to files for Genes
    Also calculates gene rela using tmb - total metagenome base pairs"""

    print('... Calculating TADs for Genes')
    gene_tad, gene_breadth, gene_rela, gene_rpkm = get_gene_tad(gn_tad, tad, tmb)
    ...
    print('... Writing Gene Relative Abundance file.')
    _ = write_file(gene_rela, gn_len, outpre, '_gene_rela.tsv', 12)

    print('... Writing Gene RPKM file.')
    _ = write_file(gene_rpkm, gn_len, outpre, '_gene_RPKM.tsv', 12)
...
def calc_intergene_stats(
    intergn_tad, intergn_anir, intergn_len, tad, outpre, precision, tmb
    ):

    """Calculate ANIr, TAD and breadth and write to Intergene files"""

    print('... Calculating TADs for Inter-Gene Regions')
    intergene_tad, intergene_breadth, igrela, igrpkm = get_gene_tad(
                                                        intergn_tad, tad, tmb
                                                        )
    ...
    print('... Writing Inter-gene Relative Abundance file.')
    _ = write_file(igrela, intergn_len, outpre, '_inter-gene_rela.tsv', 12)

    print('... Writing Inter-gene RPKM file.')
    _ = write_file(igrpkm, intergn_len, outpre, '_inter-gene_RPKM.tsv', 12)
...
def operator(
    ...
    # return total metagenome bp at tmb to calculate rela of genes
    tmb = calc_genome_stats(
    ...
    if do_genes == True:
        ...
        _ = calc_gene_stats(
            gn_tad, gn_anir, gn_len, tadp, outpre, precision, tmb
            )
        ...
        _ = calc_intergene_stats(
            intergn_tad, intergn_anir, intergn_len, tadp, outpre, precision, tmb
            )
